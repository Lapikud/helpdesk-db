...
export default function LocationEdit({
	...
	const {
		...
		defaultValues: async () => {
			const result = await locationService.getAsync(id);
			if (result.errors && result.errors.length > 0) {
				setErrorMessage(
					result.statusCode + " - " + result.errors.join(", ")
				);
				return { locationName: "", shelfNum: 0, column: 0 };
			} else {
				setErrorMessage("");
				return {
					locationName: result.data!.locationName,
					shelfNum: result.data!.shelfNum,
	...
	const onSubmit: SubmitHandler<Inputs> = async (data: Inputs) => {
		...
		try {
			const result = await locationService.updateAsync({
				id: id,
				...
				>
					...
					<div className="relative mb-4">
						...
						<input
							...
							type="text"
							id="locationName"
							placeholder={tLocation("CodeNamePrompt")}
